...
import { Calendar1Icon, Loader, Loader2 } from 'lucide-react'
...
//fiter categories based on income or expense
const filteredCategories = categories.filter(
  ...
  useEffect(()=>{
    if(transactionResult?.success&&!transactionLoading){
      toast.success(editMode?"Transaction upadted succesfully":"Transaction created Successfully");
      reset();
...
{errors.recurringInterval &&(<p className="text-sm text-red-500">{errors.recurringInterval.message}</p>)}
    ...
    <div className="flex gap-4">
      <Button type="button"
      variant="outline" className="w-full" onClick={()=>router.back()}>Cancel</Button>
      <Button type="submit" className="w-full" disabled={transactionLoading}>
        {transactionLoading?(
          <>
          <Loader2 className="mr-2 h-4 w-4 animate-spin"/>
          {editMode?"Updating...":"Creating..."}</>
        ):!editMode?("Create Transaction"):("Update transaction")}
        </Button>
    </div>
    </form>
  )
